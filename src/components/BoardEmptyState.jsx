...
import { PenLine, TrendingUp, HelpCircle, MessageCircle, Film } from 'lucide-react'
import AnimatedCounter from './AnimatedCounter'

// 게시판 글이 0개일 때 보여주는 "살아있는" 빈 화면.
// props:
//   challenge   : 이번 주 챌린지 문구 (site_settings.weekly_challenge). 비면 챌린지 숨김.
...
export default function BoardEmptyState({ challenge = '', totalVideos = null }) {
  return (
    <div className="py-6">
      {totalVideos ? (
      ...
      <div className="mb-4 rounded-2xl border border-[#03C75A]/20 bg-white p-5 shadow-sm">
        ...
        {challenge ? (
          <div className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-[#03C75A]/10 px-3 py-1.5 text-sm font-bold text-[#03C75A]">
            🎯 이번 주 챌린지 — {challenge}
          </div>
        ) : null}
        <Link
      ...
      <div className="mt-4 rounded-2xl border border-dashed border-gray-300 p-5 text-center text-sm leading-relaxed text-slate-400">
        곧 친구들의 활동(자랑하기·새 친구)이 여기에 자동으로 채워져요 ✨
      </div>
    </div>
  )
}
