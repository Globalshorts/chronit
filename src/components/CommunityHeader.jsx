      ...
      <div className={`fixed top-0 right-0 left-0 z-40 transform pt-[76px] transition-all duration-300 ease-in-out md:hidden ${menuOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'}`}>
        <div className="border-b border-gray-200 bg-white px-6 py-5 shadow-lg">
          ...
          <div className="mt-3 border-t border-gray-200 pt-3">
            {user && (
              <Link to="/generate" onClick={() => setMenuOpen(false)} className="mb-2 block rounded-xl bg-[#0064FF] px-4 py-3.5 text-center text-lg font-bold text-white">무료 체험</Link>
            )}
            {user ? (
              ...
              <Link to="/generate" onClick={() => setMenuOpen(false)} className="block rounded-xl bg-[#0064FF] px-4 py-3.5 text-center text-lg font-bold text-white">로그인 / 무료 체험</Link>
            )}
          </div>
        </div>
      </div>
