...
// 대본 백그라운드 생성 중 안내 토스트 (크로닛 다크 글래스 스타일).
// 생성 중일 때 "레드노트에서 클립 찾기"를 유도한다.
    ...
    <div className="pointer-events-none fixed inset-x-0 bottom-20 z-[2147482000] flex justify-center px-4 md:bottom-6">
      <div className="pointer-events-auto flex max-w-md items-center gap-3 rounded-2xl glass px-4 py-3 shadow-2xl shadow-black/40">
        ...
        <div className="min-w-0">
          <div className="text-sm font-bold text-white">베라가 대본을 쓰고 있어요{generating > 1 ? ` (${generating})` : ''}</div>
          <div className="mt-0.5 text-[13px] leading-snug text-white/55">이 시간에 레드노트에서 쓸 클립을 찾아보세요<br className="hidden sm:block" /> 준비되면 버튼이 <span className="font-bold text-[#5AA0FF]">‘베라에서 보기’</span>로 바뀌어요</div>
        </div>
      </div>
    </div>
  )
