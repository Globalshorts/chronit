...
export default function PwaInstall() {
  ...
  const isDesktop = !isIOS && !isAndroid && !isInApp
    ...
    <div className="fixed inset-0 z-[130] flex items-end justify-center bg-black/60 px-4 pb-6 backdrop-blur-sm sm:items-center sm:pb-0" onClick={() => setOpen(false)}>
      <div className="relative w-full max-w-sm rounded-3xl border border-gray-200 bg-white p-7 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-5 text-center">
          ...
          <h3 className="text-lg font-bold text-gray-900">{isDesktop ? '북마크에 추가하고 빠르게 열기' : '홈 화면에 추가하고 앱처럼 쓰기'}</h3>
          <p className="mt-1.5 text-sm text-gray-500">{isDesktop ? '즐겨찾기 한 번으로 바로 들어와요.' : '아이콘 한 번으로 바로 들어와요. (설치·앱스토어 불필요)'}</p>
        </div>

        {isDesktop ? (
          <div className="space-y-4">
            <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-5 text-center">
              <div className="text-sm text-gray-600">이 페이지를 북마크에 추가하려면</div>
              <div className="mt-2.5 flex items-center justify-center gap-1.5">
                <kbd className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-bold text-gray-800 shadow-sm">{isMac ? '⌘ Command' : 'Ctrl'}</kbd>
                <span className="text-gray-400">+</span>
                <kbd className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-bold text-gray-800 shadow-sm">D</kbd>
              </div>
              <div className="mt-2 text-xs text-gray-400">키를 누르면 브라우저 북마크 창이 열려요</div>
            </div>
            <button onClick={copyLink} className="flex w-full items-center justify-center gap-1.5 rounded-2xl border border-gray-300 px-4 py-3 text-sm font-bold text-gray-700 hover:bg-gray-50 active:scale-[0.98]">{copied ? (<><Check size={15} /> 복사됨</>) : (<><Copy size={15} /> 주소 복사</>)}</button>
          </div>
        ) : isInApp ? (
